...
(function () {
  ...
  // ---------- deployment stamp ----------
  // Which build is this? The daily update regenerates the market data and
  // rebuilds the image in the same run, so the data's generation date is also
  // the deployment's date -- exactly what a bug report needs in order to say
  // whether it came from a current or a stale instance. (A build made without
  // re-running the pipeline shows the committed data's date, which is still the
  // truth about what that build contains.) The tooltip carries the detail.
  function stampBuild() {
    const el0 = $("buildStamp");
    if (!el0 || !DATA || !DATA.meta) return;
    el0.textContent = "Updated " + DATA.meta.generated;
    let tip = "Market data " + DATA.meta.firstYear + "–" + DATA.meta.lastYear +
      ", generated " + DATA.meta.generated;
    if (CAPE && CAPE.latest && CAPE.latest.date) tip += " · CAPE as of " + CAPE.latest.date;
    tip += ". Regenerated by the daily update, so this is also when this copy of the site was built.";
    el0.title = tip;
  }
